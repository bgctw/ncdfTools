...
{  
  var.name         <- setdiff(ncdf.get.varinfo(file.con, order.var ='id')$name, ncdf.get.diminfo(file.con)$name)
  if(length(var.name) > 1) {
    ...
    var.ind <- var.id.nocoord[which.max(ncdf.get.varinfo(file.con)[var.id.nocoord,4])] +1
    if (length(var.id) > 1) {
