gapinfo.temporal.ncdf = function(
...
##              the data cube of a ncdf file. 
        ...
        , grids.exclude = NULL ##<< object to exclude grid cells (as, for example, ocean tiles) from the 
                               ...
                               ## and latitude consisting of vectors with the indices along the repective
                               ## dimension of grid cells to exclude. A last optzion is to supply a matrix of 
                               ## logical values with its dimensions beeing identical with the spatial 
                               ## dimensions in the ncdf file.
        , col.palette=colorRampPalette(c('blue','yellow','red')) 
                               ##<< color palette to use for the maps, i.e. a function taking a single integer 
                               ## as an argument and returning that many hexadecimal rgb values.
)
...
##value<< nothing is returned but plots of the gap statistics are produced.
{
    ...
    if (!dim(ncdf.get.diminfo(file.con))[1] == 3)
        stop('Only processing of three dimensional ncdf files is implemented!')
    if (sum(is.na(match(c('time','longitude','latitude'),ncdf.get.diminfo(file.con)[,'name'])))>0)
    ...
    dims.lengths    <-  ncdf.get.diminfo(file.con)$length
    dims.names      <-  ncdf.get.diminfo(file.con)$name    
    if (is.null(grids.exclude)) {
        grids.valid.matrix <-  array(TRUE,dim = dims.lengths[ - dim.time])
    } else if (is.function(grids.exclude)) {
        grids.valid.matrix <- !apply(data,MAR=(1:3)[ - dim.time], FUN = grids.exclude)
        ...
        stop('class of grids.exclude not suppoerted!')
    }
    
    #compute spatial maps
    dims.spatial.id <-  ncdf.get.diminfo(file.con)$id[ - match('time'
                             , ncdf.get.diminfo(file.con)$name)]   
    gap.stat.spatial<-  apply(data, MAR = dims.spatial.id + 1
                             , function(x){dummy=temporalGaps(x);c(dummy$mean[1],dummy$max[1])})
    gap.stat.spatial[array(rep(!grids.valid.matrix,each=2),dim=c(2,dim(grids.valid.matrix)))]<- NA                 
    
    #compute global averages                 
    dims.order      <-  c(match('time', ncdf.get.diminfo(file.con)$name), (1:3)[  -  dim.time])
    ...
    if (length(ind.groups) > 0){
        ...
        ind.groups.vector <-  rep(1, times = prod(ncdf.get.diminfo(file.con)$length))
    }
    gap.stat.global     <-  temporalGaps(series = data.clean, index = ind.groups.vector
                , results.ext = TRUE)
