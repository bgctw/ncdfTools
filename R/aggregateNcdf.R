aggregateNcdf <- function(
  ...
  fileNameOut <- file.path(
    path.out
    , sub('[.]nc', paste('_', period, '.nc', sep = ''), sub('.*/', '', fileName)))
  cdoCmd      <- paste('cdo ', cdoOperator, ' ', fileName, ' ', fileNameOut, sep = '')
  ##run aggregation
  system(cdoCmd)
  message('Created file ', fileNameOut)
  ##value<<
